...
namespace Api.Controllers
{
    ...
    public class RequestController : ControllerBase
    {
        ...
        {
            var result = await _command.AddAsync(request, User.GetUserId());
            if (result.IsSuccess)
                return Ok(result);
        ...
        public async Task<IActionResult> GetAllRequests(int? categoryId, int pageNumber = 1, int pageSize = 10)
        {
            var result = await _query
                .GetAllAsync(categoryId, pageNumber, pageSize, User.GetUserId(), User.GetRole());
            return Ok(result);
        }
        ...
        {
            var result = await _query.GetRecentActivity(User.GetUserId(), User.GetRole());
            return Ok(result);
        }
        ...
        {
            var result = await _query.GetDashboardStatsAsync(User.GetUserId(), User.GetRole());
            return Ok(result);
        }
    }
}